...
# Environment variables expected in .env (dbuser, dbpassword, dbhost, dbname, dbport)
db_user = os.getenv("DBUSER") or os.getenv("dbuser")
db_password = os.getenv("DBPASSWORD") or os.getenv("dbpassword")
db_host = os.getenv("DBHOST") or os.getenv("dbhost", "localhost")
db_name = os.getenv("DBNAME") or os.getenv("dbname")
db_port = os.getenv("DBPORT") or os.getenv("dbport", "3306")

if not all([db_user, db_password, db_host, db_name]):
    raise ValueError("Missing database configuration in .env file (DBUSER/DBPASSWORD/DBNAME required)")

DATABASE_URL = f"mysql+pymysql://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"

# create engine compatible with SQLAlchemy 2.x
engine = create_engine(DATABASE_URL, future=True, pool_pre_ping=True)

# sessionmaker for modern SQLAlchemy
...
def seed_states():
    ...
    insert_query = text("""
        ...
        VALUES (:name, :capital, :region, :slogan, :population, :landmarks)
    """)

    try:
        # Use a transactional connection for seeding
        with engine.begin() as conn:
            count = conn.execute(text("SELECT COUNT(*) FROM states")).scalar()
            if not count:  # Only seed if table is empty
                for state in STATES:
                ...
                print("ℹ️ States table already has data. Skipping seeding.")
    except Exception as e:
        print(f"Error seeding states: {e}")


def init_db() -> None:
    """Create tables (if not exist) and seed default data."""
    try:
        # Create tables from metadata (no-op if tables exist)
        Base.metadata.create_all(bind=engine)
        # Seed states (seed_states uses engine.begin internally)
        seed_states()
